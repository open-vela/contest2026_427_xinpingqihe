...
| **蓝牙：B1/B2/B3 全部打通（2026-09-18 晚）** | **根因①**：zblue 的 NuttX port 把 HCI 命令 `write()` 放在 Zephyr 系统工作队列线程里，而 NuttX 的 **fd 表挂 `task_group`**（`fs/inode/fs_files.c` `file_get2 → nxsched_get_fdlist → tcb->group->tg_fdlist`），该线程与 `h4_open()` 所在组不同 ⇒ 每次 `write()` 都 **EBADF(errno 9)**；`hci_core.c` 在 `bt_send` 失败后用 `hci_cmd_done(..BT_HCI_ERR_UNSPECIFIED)` **伪造** status 0x1F 再翻成 `-EIO` ⇒ 表象是「控制器没回包」，其实**根本没发出去**。**修法①**：`port/drivers/bluetooth/hci/h4.c` 的 `h4_send()` 只入 `tx_fifo`，真正 `write()` 交给 `h4_rx_thread`（与 open 同组）。**根因②（B3 收尾时靠空口字节级 trace 才抓到，比①隐蔽）**：设备侧 `[bt] notify #N rc=0 len=16` 连打 56 条一切正常、宿主侧**一个包都收不到** —— 打开 `PW_H4_TRACE` 后用新写的 `tools/phywear/pw_h4_notify_decode.py` 解码，通知帧是 `… 1b 00 00 …`（value handle **0x0000**），BlueZ 找不到该句柄的特征 ⇒ **静默丢弃**。代码级根因在 zblue：上游 Zephyr 在 `bt_gatt_notify_cb()` 里**就地**在已填好 `data.handle` 的 `struct notify_data` 上做 CCC 遍历，而 zblue 为多控制器抽出的 `gatt_notify_mc()` 里 `struct notify_data data = { 0 };` **漏了 `data.handle = handle;`**（`gatt_indicate_mc()` 同）。**修法②**：`subsys/bluetooth/host/gatt.c` 补两行 `data.handle = handle;`（语义与上游一致）。**实测**：修复前 56/56 条句柄 0x0000、宿主 0 包；修复后 2 条句柄 0x0010、宿主 2 包。**B3 已完成且无人化**：VM 蓝牙启用后宿主（Intel AX201 / hci0）自己当 BLE 中心设备，`python3 tools/phywear/pw_bt_b3.py --central --out <dir>` 一条命令跑完复位→`phywear cap bt`→扫描/连接/读 16 B/订阅收包/写 'ping'→双向判定，**设备侧 6/6 + 宿主侧 9/9，退出码 0**；出货构建（诊断开关 `PW_H4_TRACE`/`PW_BT_NOTIFY_TRACE` 均归 0）上**复跑仍全过**。⚠️ 本队**没在真手机（Android/iOS）上点过** —— 如实说明：用的是宿主 BlueZ 中心设备，动作与手机上 nRF Connect 完全对应；手机路径（`--wait 240`）保留可作独立交叉验证。⚠️ 本板广播用**随机地址**（EXT_ADV+PRIVACY），**按名字 `PhyWear` 找，不能按 MAC**。断线后广播**自动重开**（`BT_ADV_PERSIST=true`）。新工具：`pw_ble_central.py`（BlueZ+D-Bus 中心设备，不需要 `bleak`）、`pw_h4_notify_decode.py`（`[H4] TX` 分层解码 + `--expect-handle` 断言）。新坑：① 自动跑 B3 时用 `subprocess.run()` 死等会**不读串口** ⇒ 设备侧证据全留在 tty 缓冲区（宿主侧全 PASS、设备侧报「没连上」）；必须边等边 `pump()` —— **串口取证器不读就等于没有证据**；② 扫描前必须清掉 BlueZ 里同名的**缓存设备记录**（随机地址每次上电可能变），否则会连到早已不广播的死地址；③ 收 `PropertiesChanged` 必须把 D-Bus 连接挂到 GLib 主循环。证据 `docs/evidence/bt-b3-20260918/` + `docs/evidence/bt-gatt-handle-20260918/`（均含 `SHA256SUMS.txt`），详见 `docs/16 §12` + `docs/03 §4.10c` |
