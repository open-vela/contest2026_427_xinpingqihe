...
| **蓝牙：B1/B2 已打通（2026-09-18）** | **根因**：zblue 的 NuttX port 把 HCI 命令 `write()` 放在 Zephyr 系统工作队列线程里，而 NuttX 的 **fd 表挂 `task_group`**（`fs/inode/fs_files.c` `file_get2 → nxsched_get_fdlist → tcb->group->tg_fdlist`），该线程与 `h4_open()` 所在组不同 ⇒ 每次 `write()` 都 **EBADF(errno 9)**，控制器一个字节都没收到；`hci_core.c` 在 `bt_send` 失败后用 `hci_cmd_done(..BT_HCI_ERR_UNSPECIFIED)` **伪造** status 0x1F，再被翻成 `-EIO` ⇒ 表象是"控制器没回包"，其实是**根本没发出去**。**修法**：`port/drivers/bluetooth/hci/h4.c` 的 `h4_send()` 只入 `tx_fifo`，真正 `write()` 交给 `h4_rx_thread`（与 open 同组）。**实测**：`[bt] bt_enable rc=0 (host stack up) elapsed≈1.2s`；`B2 gatt register/conn cb/adv start rc=0`；`READY name=PhyWear`；服务端自检 `read sensor 16B |a|=1013 mg / write cmd ok / SELFTEST PASS`。**B3（手机连接读写）仍未做 —— 需人工**，用 `python3 tools/pw_bt_b3.py --out <dir> --wait 240`（会自动判 5 项 PASS/FAIL；负对照已做；它**发完 `cap bt` 会等到 `adv start rc=0` 才让你去连手机**，因为本板控制台被 NSH 与 `vela>` 抢 stdin，命令可能被吃掉）。另：`phywear cap bt` **会把 GUI 一起带起来**（心跳 `fps=3` 就是 root 页，属正常区间 3~7），所以一个 boot 的 GUI 额度会被它用掉；浸泡 185 s 无 panic/重启（`27_soak-185s-bt-alive.log`）。⚠️ 本板广播用**随机地址**（EXT_ADV+PRIVACY），**手机按名字 `PhyWear` 找，不能按 MAC**。断线后广播会**自动重开**（`BT_ADV_PERSIST=true` + `conn.c:1576` 调 `bt_le_adv_resume()`，代码级已证、空口未验）⇒ 可以反复连，不用复位板子。详见 `docs/16` + `docs/evidence/bt-h4-20260918/` |
