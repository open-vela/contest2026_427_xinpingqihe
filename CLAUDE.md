...
| **任务3 网络：本板硬件不可达（已证）** | SF32LB52 黄山派**没有无线网卡**：运行时 `ifconfig` → `ifconfig: open failed: 2`（ENOENT，命令在网卡不在）；`/dev` 无任何 wlan/eth；defconfig 无 `CONFIG_WIFI`/`IEEE80211`；厂商树无任何 WiFi 芯片驱动、无 `netdev_register`。ai_agent 的 `set_wifi` 走 `ifup wlan0` + `wapi`，**需要真实网卡**。⇒ N1/N2 在本板不可达，只能换带无线的板（r528/gemini-s1）或写进已知限制。**不许写成已实现** |
