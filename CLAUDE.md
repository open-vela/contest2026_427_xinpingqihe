...
| 真机固件 | 板上 = **UI v2 构建**（**2,486,532 B** / flash 14.82% / SRAM **489,136 B**；本地产物 md5 `256e0612f3548f870310f2789f0fb6a1` —— ⚠️ 固件**非可重现**，md5 只在「板上 vs 同一产物」时有意义）。含蓝牙阶段 A、P1-2 动效查表（151 vs 2088 ns/次）、P0 主循环自适应休眠、UI v2（色板 + 统一按压态 + 切屏转场 + 描边）。⚠️ `flash_with_ftab.sh` 读回校验会把 `sftool` 读取**超时**误报成「固件 ❌ 不一致」（stderr 被 `>/dev/null` 吞掉），本轮复现多次，实际写入正常（板子正常启动跑 GUI，截图已核）。上一版 `6c80b77e…`（2,486,288 B，UI 首批）；更早回退点 `~/桌面/PhyWear-rollback-20260915-0002/` |
