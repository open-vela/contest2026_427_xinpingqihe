...
rt_err_t drv_lcd_fb_write_send(LCD_AreaDef *write_area, LCD_AreaDef *src_area, const uint8_t *src, write_fb_cbk cb, uint8_t send)
{
    ...
    if (area_intersect(&common_area, write_area, src_area))
    {
        ...
        /* PhyWear P0 实验开关（2026-09-16）：防撕裂行等待。
         * 默认 0 = vendor 原行为（等 LCDC 扫描到目标行再写，避免撕裂）。
         * 置 1 = 不等，直接写：实测能拿走每帧固定的十几毫秒，但**可能可见撕裂**，
         * 需人眼确认。回退：改回 0 即完全恢复。

         * 机理：wait_line_valid() 阻塞在 LCDC 的逐行完成事件上，等待时间取决于
         * 面板扫描进度，与本次要写的脏区高度无关 —— 这正是"不同页面单帧耗时都≈17~21 ms"
         * 的原因（实时页 17.1 ms / 图线页 19.2 ms / 轨迹页 21.4 ms）。 */

#ifndef PW_LCD_SKIP_TEAR_WAIT
#  define PW_LCD_SKIP_TEAR_WAIT 0   /* 实测：跳过反而更差（45 vs 48 fps），保持原行为 */
#endif

#if !PW_LCD_SKIP_TEAR_WAIT
