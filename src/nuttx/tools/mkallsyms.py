...
class SymbolTables(object):
    ...
    # NOTE(phywear): 这里的逻辑原本是反的 —— `--noconst` 名字上表示"不要 const"，
    # 但默认分支却生成了非 const 表，于是 g_allsyms（只读符号表，5740 项 ≈45.8 KB）
    # 落进 .data：既占 flash 镜像又占一份 SRAM。消费方 symtab_allsyms.c 本来就按
    # `extern const struct symtab_s g_allsyms[]` 使用，所以生成 const 反而是一致的。
    # 改成默认 const（→ .rodata → flash），`--noconst` 保持"非 const"语义。
