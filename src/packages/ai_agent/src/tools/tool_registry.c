...
int tool_registry_init(void)
{
    ...
    /* PhyWear wrist physics workshop tools */
    REGISTER_TOOL_NO_PARAMS("phywear_list_experiments",
        "List the PhyWear physics experiment screens available on this watch, "
        "with their names and what each one measures.",
        tool_phywear_list_execute);

    REGISTER_TOOL("phywear_open_screen",
        "Bring a PhyWear experiment page up on the watch display. Use the "
        "screen names reported by phywear_list_experiments.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_STR("screen", "Screen name, e.g. pendulum or spec_accel")
                TOOL_SCHEMA_END_REQUIRED("\"screen\""),
        tool_phywear_open_execute);

    REGISTER_TOOL("phywear_read_sensor",
        "Read one of the PhyWear board sensors right now.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_ENUM("sensor", "Which sensor to read",
                            "\"accel\",\"mag\",\"light\"")
                TOOL_SCHEMA_END_REQUIRED("\"sensor\""),
        tool_phywear_sensor_execute);

    REGISTER_TOOL("phywear_run_experiment",
        "Open a PhyWear experiment page and sample the accelerometer for a few "
        "seconds, returning min/max/mean values the agent can reason about.",
        TOOL_SCHEMA_BEGIN()
            TOOL_PARAM_STR("screen", "Experiment screen name, e.g. pendulum")
            "," TOOL_PARAM_NUM("seconds", "Sampling window in seconds, 1-30")
                TOOL_SCHEMA_END_REQUIRED("\"screen\""),
        tool_phywear_run_execute);
