...
/* 总线是否已初始化（即 ai_agent 是否已起来）。
 * 供 velaclaw 本地客户端做安全探测：总线没初始化时往队列 push 会在
 * pthread_mutex_take 里 DEBUGASSERT(NXSEM_IS_MUTEX) 直接 panic，
 * 所以必须在 open() 阶段就拒绝，而不是等到 push 时炸。 */

bool message_bus_ready(void);
