			  ...
			  struct bt_gatt_notify_params *params)
{
	...
	/* notify_cb() 用 data->handle 当"特征值句柄"填进 ATT Notification PDU。
	 * 上游 Zephyr 是在同一个 notify_data 上就地做 CCC 遍历（句柄自然带过去），
	 * 这里被重构成独立函数后漏了这一行 —— 结果 data.handle 恒为 0，
	 * 空口上真的会发出 `1b 00 00 ...`（句柄 0x0000）的通知帧，
	 * 对端（BlueZ）找不到句柄 0x0000 的特征而**静默丢弃**：
	 * 设备侧 rc=0/notify 计数正常，手机侧永远收不到。 */
	data.handle = handle;
			    ...
			    struct bt_gatt_indicate_params *params)
{
	...
	params->_ref = 0;
	/* 同 gatt_notify_mc()：句柄必须带进 notify_cb，否则成帧句柄为 0 */
	data.handle = handle;
