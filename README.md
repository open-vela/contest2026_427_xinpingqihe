...
- 蓝牙（2026-09-18 新增，**B1/B2 已打通、真机实测**）：片内 LCPU 控制器 + zblue host 栈跑通 ——
  `phywear cap bt` 打印 `[bt] bt_enable rc=0`，注册 GATT 服务（`e0f1a000-…`，含传感器读数 16 B
  通知特征 + 命令写入特征）并开可被发现广播（名字 `PhyWear`）；服务端自检 `SELFTEST PASS`。
  ⚠️ **手机连接读写（B3）仍需人工实测**；⚠️ 本板**没有 WiFi 网卡**，`set_wifi`/DHCP 在本板不可达。
  详见 `docs/16_蓝牙B1根因与修复.md`（含根因：NuttX fd 表按 `task_group`）。
