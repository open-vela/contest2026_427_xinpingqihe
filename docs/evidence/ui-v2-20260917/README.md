...
- **fps 五档完全相同（12）**；累计 loops/s −6.3%（描边 ≈1.5%、读数页 v2 版式 ≈2.5%）。**帧率不退化**，视觉提升保留。
- flash：2,485,140 → **2,487,420 B**；**SRAM 489,136 B 未变（0 B 增长）**。
- 板上固件 = 本地产物 md5 见 `CLAUDE.md` 真机固件行（2,487,420 B）。
