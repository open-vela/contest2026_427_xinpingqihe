# 演示视频素材与分镜（2026-09-15 起，2026-09-18 晚更新）

> 对应六项计划的第 6 项（演示视频）。

## 0. 现状（2026-09-18 晚）：**已有 v1 成片**

| 文件 | 时长 | 规格 | 说明 |
|---|---|---|---|
| **`phywear-demo-v1.mp4`** | **4:44（284 s）** | 1280×720 · H.264 · yuv420p · 25 fps · 无音轨 · **4.84 MiB** | **当前成片**：24 张分镜、底部字幕、入场推近 |
| `phywear-demo-v0.1.mp4` | 4:55（295 s） | 同上（3.9 MiB） | 09-15 的**静态素材样片**，已被 v1 取代（保留作过程记录） |

```bash
sha256sum phywear-demo-v1.mp4
# 41921766832fcd5a35087d9b255a8bd628c528c080c75ba27b063e310412510c
```

**v1 的画面来源（片内也常驻声明，不靠口头）**：
真机截图（26+ 张，含本轮蓝牙 4 张）+ 真机采集数据 + 宿主实测（桌面 GUI）+ 串口/空口原文卡；
动态部分是**对整帧做缓慢推近/平移（后制）**，不是实拍动态画面 —— 真机无视频输出。

**v1 相对 v0.1 的实际改进**：
1. 加了**底部字幕条**（本片无音轨，信息全走字幕；取自 docs/08，且已按最新事实更新）；
2. 加了**入场推近**（每张分镜开头 0.5 s 推一下，其余时间静止）；
3. **把蓝牙从"no-go 结论"改成了本轮真实结果**：连接标识（灰/蓝）、手表版串口页、
   两个根因的字节级 A/B、桌面一键测试 GUI（14/14）—— 这是本轮最大的增量；
4. 删掉两张**模拟器对照**分镜（15 s + 8 s，演示价值低），时间让给蓝牙；
5. 修正了**过时数字**（第 8 段的固件/SRAM 口径曾写 2,084,220 B / 446,640 B / 85.19%，
   现按当前固件改为 2,554,544 B / 496,412 B / 94.68%）；
6. 校验仍然是硬的：缺素材**直接报错退出**、文字溢出**直接报错**、总时长 ≤300 s、
   编码必须 h264/yuv420p，最后**全片解码一遍**确认能从头播到尾。

**v1 仍然缺什么（如实标注，不假装有）**：**实拍动态画面**与**真人讲稿录音**。
§1 的结论没变：真机没有视频输出、模拟器取帧 0.3 fps、本机原先没有 ffmpeg ⇒
动态画面只能由人拿手机拍 AMOLED 屏。§3 的 4 条"需要人手上的动作"、
§5 末尾的 10 条拍摄待办，就是**实拍版**要补的镜头清单。

---

## 0b. 为什么早先这里没有视频文件（2026-09-15 的记录，保留）
...
## 5. v0.1 样片（2026-09-15，已被 v1 取代）—— **静态素材版，不是实拍**
